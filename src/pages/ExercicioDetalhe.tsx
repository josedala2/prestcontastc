...
const ExercicioDetalhe = () => {
    ...
    return (
      <AppLayout>
        <div className="flex flex-col items-center justify-center py-20 text-muted-foreground">
          ...
          <Button variant="outline" className="mt-4" onClick={() => navigate("/exercicios")}>
  ...
  return (
    <AppLayout>
      <div className="mb-6">
        <Button variant="ghost" size="sm" className="gap-1.5 text-muted-foreground mb-3" onClick={() => navigate("/exercicios")}>
          <ArrowLeft className="h-4 w-4" /> Voltar aos Exercícios
        </Button>
        <div className="flex items-start justify-between flex-wrap gap-4">
          <div>
            <div className="flex items-center gap-3 mb-1">
              <h1 className="text-2xl font-bold text-foreground">{fy.entityName} — {fy.year}</h1>
      ...
      <Tabs defaultValue="geral" className="space-y-4">
        <TabsList className="flex-wrap h-auto gap-1">
          ...
          <TabsTrigger value="balanco" className="gap-1.5"><Landmark className="h-3.5 w-3.5" /> Balanço Patrimonial</TabsTrigger>
        ...
        <TabsContent value="geral" className="space-y-4">
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
            ...
            <Card>
              <CardHeader className="pb-2"><CardTitle className="text-sm text-muted-foreground">Progresso</CardTitle></CardHeader>
              <CardContent>
        ...
        <TabsContent value="mapas" className="space-y-4">
          <Card>
            <CardHeader><CardTitle className="text-base">Modelos de Prestação de Contas (Resolução 1/17)</CardTitle></CardHeader>
            <CardContent>
              <Table>
                <TableHeader>
                  <TableRow>
                  ...
                  {[
                    ...
                    <TableRow key={m.n}>
                      ...
                      <TableCell className="text-right">
                        <Button variant="ghost" size="sm" className="text-xs" onClick={() => navigate("/mapas")}>
                          {m.filled ? "Ver" : "Preencher"}
                        </Button>
                      </TableCell>
                    </TableRow>
                  ))}
        ...
        <TabsContent value="documentos" className="space-y-4">
          <Card>
            <CardHeader className="flex flex-row items-center justify-between">
              <CardTitle className="text-base">Checklist Obrigatório</CardTitle>
          ...
          <Card>
            <CardHeader><CardTitle className="text-base">Anexos Carregados</CardTitle></CardHeader>
            <CardContent>
              <Table>
                <TableHeader>
                  <TableRow>
                ...
                <TableBody>
                  {mockAttachments.map((a) => (
                    <TableRow key={a.id}>
                      ...
                      <TableCell className="text-xs">v{a.version}</TableCell>
                      <TableCell className="text-right">
            ...
            <Card key={level}>
              ...
              <CardContent>
                {validationsByLevel[level].length === 0 ? (
                  <p className="text-sm text-muted-foreground">Sem resultados neste nível.</p>
                ) : (
                  <div className="space-y-2">
                ...
                <Card key={cr.id}>
                  <CardHeader>
                    <div className="flex items-start justify-between">
                      <div>
                        ...
                        <p className="text-xs text-muted-foreground mt-1">Criado: {cr.createdAt} · Prazo: {cr.deadline}</p>
                      </div>
                      <StatusBadge
                  ...
                  <CardContent className="space-y-3">
                    ...
                    {cr.responses?.map((r, i) => (
                      <div key={i} className="p-3 bg-muted/30 rounded-lg border-l-2 border-muted-foreground/30 ml-4">
