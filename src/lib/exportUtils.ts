...
export async function generateDossierZip(entityName = "ENDE, E.P.", year = 2024) {
  ...
  autoTable(indexPdf, {
    ...
    body: [
      ["Entidade", entityName],
      ["Exercício", `${year}`],
      ["Total Débito", `${formatKz(trialBalanceData.reduce((s, l) => s + l.debit, 0))} Kz`],
      ["Total Crédito", `${formatKz(trialBalanceData.reduce((s, l) => s + l.credit, 0))} Kz`],
      ["Registos no Balancete", `${trialBalanceData.length}`],
      ["Registos de Auditoria", `${auditLogData.length}`],
